...
//--------------------------------------------------------------------------------------------------
// searchneighbors
void BasicSPH::searchBoundaryNeighbors()
{
...
//--------------------------------------------------------------------------------------------------
// buildFluidNeighbors
void BasicSPH::buildFluidNeighbors()
{
	int max=0;
    // Reserve space and initialize neighbors' data
    _neighbors.clear();
...
//--------------------------------------------------------------------------------------------------
// buildBoundaryNeighbors
void BasicSPH::buildBoundaryNeighbors()
{
    // Reserve space and initialize neighbors' data
...
void BasicSPH::computeDensityAndPressure()
{
    ...
    for (long p=0; p<_particles.size(); ++p)
    {
		...
		for (long n=0; n<_boundaryneighbors[p].size(); ++n)
		{
            const Neighbor &boundaryneighbor = _boundaryneighbors[p][n];
			const SPHParticle &boundaryneighborparticle = _boundaryparticles[boundaryneighbor.id];

            // Add contribution
            particle.density += boundaryneighborparticle.Psi* getKernelValue(boundaryneighbor.dist);
		}
    ...
    for (long p=0; p<_particles.size(); ++p)
    {
        ...
        for (long n=0; n<_neighbors[p].size(); ++n)
        {
            ...
            if (uij < 0)
            {
                ...
                double IIij = (2*alpha*uij*speedOfSound + beta*uij*uij) / avgDensity;
...
void BasicSPH::computePressureForces()
{
    ...
    for (long p=0; p<_particles.size(); ++p)
    {
		...
		// Get boundaryneighbors contributions
        // fp_i/rho_i = -SUM_b (Psi_b * (pi/rho_i^2 ) * gradient(Wij)
        for (long n=0; n<_boundaryneighbors[p].size(); ++n)
        {
            const Neighbor &boundaryneighbor = _boundaryneighbors[p][n];
            const SPHParticle &boundaryneighborparticle = _boundaryparticles[boundaryneighbor.id];
